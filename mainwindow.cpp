...
{
    bool ok;
    QString name = QInputDialog::getText(this, QStringLiteral("发布商品"), QStringLiteral("商品名称："), QLineEdit::Normal, "", &ok);
    if (!ok || name.isEmpty()) return;

    double price = QInputDialog::getDouble(this, QStringLiteral("价格"), QStringLiteral("请输入价格："), 0, 0, 10000, 2, &ok);
    if (!ok) return;

    QString desc = QInputDialog::getMultiLineText(this, QStringLiteral("商品描述"), QStringLiteral("请详细描述商品情况："));

    Product newProduct(name, price, desc, QStringLiteral("当前用户"));
    m_products.append(newProduct);
    m_userHistory.append(newProduct); // 添加到用户历史记录
    refreshProductList();
    updateRecommendations();
}
...
void MainWindow::showProductDetails(QListWidgetItem* item)
{
    ...
    m_userHistory.append(product); // 添加到用户历史记录
    updateRecommendations();
}
...
{
    m_recommendations = m_recommender.generate_recommendations(m_products, m_userHistory);
